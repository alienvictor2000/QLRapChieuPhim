...
public class StatisticController implements Initializable {
	...
	// init chart
	Series<String, Number> sum_series = new Series();
	Series<String, Number> movie_series = new Series();
	Series<String, Number> service_series = new Series();
	...
	private void KhoiTaoBieuDo() {
		...
		revenueTypeMenu.getItems().get(0).setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				Title = "Tổng doanh thu tháng " + choosenMonth + "/" + choosenYear;
				lineChart1.setTitle(Title);
				System.out.println(choosenMonth);
				setUpSumChart(choosenMonth, choosenYear);
	...
	private void setUpSumChart(int choosenMonth2, int choosenYear2) {
		...
		//System.out.println(choosenYear2);
		//System.out.println(choosenMonth2);

		try {

			startDate = new SimpleDateFormat("yyyy-MM-dd")
					.parse(choosenYear2 + "-" + choosenMonth2 + "-1");
		...
		for (int j = 0; j <= listDate_sum.size() - 1; j++) {
			if ((listDate_sum.get(j).getMonth() == choosenMonth2 - 1)
				...
				System.out.println(listDate_sum.get(j).getDate() + " " + listIncome_sum.get(j));
			}
		}
